...
void temp_plt_Vmax()
{
	...
	TH2F *h1 = new TH2F("TRACS","",4000,-10.5,60.5,1000,-1,1000);
	chain1->Draw("Vmax*1e3*0.53:(z*1e3)>>TRACS");
		
	TTree *chain2 = (TTree *)f_data->Get("edge");
	TH2F *h2 = new TH2F("data","",4000,-10.5,60.5,1000,-1,1000);
	chain2->Draw("-Vmin*1e3:((z*1e3)-11953)>>data","Vbias==-${VBIAS}");

	TTree *chain3 = (TTree *)f_sim2->Get("edge");
	TH2F *h3 = new TH2F("TRACS2","",4000,-10.5,60.5,1000,-1,1000);
	chain3->Draw("Vmax*1e3*0.53:(z*1e3)>>TRACS2");
	...
	leg1->Draw();
	c->Print("Vmax_hpk_lgad_com_TF_bv${VBIAS}.pdf");
